...
while True:
    ...
    if keys[pygame.K_a] and walled != -1:
        accelx = (0.88841 - int(not(grounded)) * 0.373)
        Player.vel[0] = Player.vel[0] * (0.42 if Player.vel[0] >= 0 else 1)
        Player.vel[0] -= accelx if Player.vel[0] - accelx >= -4.2962 else 0
    ...
    if keys[pygame.K_f] and not walled:
        if Player.dashable:
            ...
            dash_velx = 8.13413998694 * int(not bool(abs(Player.vel[0]) < 0.878)) * p_side + (0.361 * relus(grounded) * p_side) + (0.01 * relus(walled) * p_side)
            dash_vely = -0.83796337997 * int(not(grounded)) + abs(Player.vel[1]) / -4.246
    ...
    if grounded:
        ...
        Player.vel[0] -= Player.vel[0] * dt * 0.12832
        if lgrounded == False:
            Player.vel[0] *= 0.9
    ...
    Player.apply_air_res(0.912314, 0.99234)
    ...
    Player.offset = Vector2((_p_w - p_w) / 2, (_p_h - p_h))
    Player.scale = Vector2(p_w, p_h)

    Camera.follow(fol_obj, 0.111)
